...
if [ "$1" = 'DB' ]; then
    ...
    echo "PROXYCACHE_UPSTREAM_REGISTRY: ${PROXYCACHE_UPSTREAM_REGISTRY}"
    # setup PROXYCACHE_UPSTREAM_REGISTRY in github actions environment variable
    # example: PROXYCACHE_UPSTREAM_REGISTRY=docker.io or PROXYCACHE_UPSTREAM_REGISTRY=jfrog-artifactory
    # if PROXYCACHE_UPSTREAM_REGISTRY is not set, default to registry.goharbor.io
    if [[ "${PROXYCACHE_UPSTREAM_REGISTRY}" == *docker* ]]; then
        PROXY_CACHE_TESTCASE_TAG="proxy_cache_from_dockerhub"
    else
