...
class TestProxyCache(unittest.TestCase):
    ...
    def do_validate(self, registry_type):
        ...
        #11. Image (often a multi-arch tag) pulled by ctr CLI should be cached;
        # ctr negotiates a single-platform manifest, so Harbor may store manifest.v2 / OCI image
        # manifest with references=None — unlike docker pull of a manifest list (step 10).
        ...
        self.assertIsNotNone(ret_index_by_c)
        if ret_index_by_c.references is not None:
            self.assertEqual(len(ret_index_by_c.references), 1)
        else:
            self.assertIn(
                ret_index_by_c.manifest_media_type,
                (
